...
```js
// 连接新的数组,可以在分页的时候使用,每次请求到新的分页的数据,添加到旧的数据中
...
// ?.运算符，直接在链式调用的时候判断，左侧的对象是否为null或undefined.如果是的就不在往下运算，返回undefined
```

###### 拷贝

```js
// 这种不是拷贝，相当于两个变量指向同一个地址.修改a中的值，b也跟着改变
var a = {name: 'hcp'}
var b = a
...
```js
let info={name: 'hcp', age:12}

// 如果参数为对象，可以在参数中直接解构出来
