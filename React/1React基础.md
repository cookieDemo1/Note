...
export default class LifeComponent extends React.Component{
   ...
   *    shouldComponentUpdate: 手动返回true或者false, 返回true代表元素允许改变，false代表不允许改变
...
- 表单受控组件有时候可能会很麻烦,因为每一个input都需要绑定onChange事件,当input框变多之后,就会绑定很多事件
- 大多数情况下,推荐使用受控组件
...
export default class Form1 extends React.Component{
  ...
  // 4.表单onSubmit事件绑定的函数
  handleSubmit = (event)=>{
    // 阻止表单默认的事件（因为提交之后会进行页面跳转，我们不希望它跳转）
...
// 2.使用container组件，并传递两个组件About和Mine组件
import React from 'react'
...
const Ucent = (props) => {
  ...
  return(
    <div>
      <Container top={<About></About>} bottom={<Mine></Mine>}></Container>
    </div>
  )
}
